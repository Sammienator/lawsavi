...
import logo from '../assets/lawsavic logo.png'
  ...
  return (
    <header className="fixed top-0 w-full bg-bg-light/95 dark:bg-bg-dark/95 backdrop-blur-lg z-50 shadow-shadow-light dark:shadow-shadow-heavy transition-all duration-300">
      <nav className="flex justify-between items-center px-4 sm:px-6 max-w-7xl mx-auto py-4">
        <motion.a
          href="#home"
          className="text-2xl font-bold text-primary-color dark:text-accent-color flex items-center"
          initial="hidden"
          animate="visible"
          variants={slideInLeft}
        >
          {isDarkMode ? (
            <img src={logo} alt="Lawsavic Investment Limited Logo" className="w-32 h-32" />
          ) : (
            <>
              <img src={logo} alt="Lawsavic Investment Limited Logo" className="w-24 h-24 mr-2" />
        ...
        <ul className="hidden sm:flex list-none gap-8 items-center max-md:gap-4">
          ...
          <li>
            <button
              onClick={toggleTheme}
              className="p-2 rounded-full w-10 h-10 flex items-center justify-center hover:scale-110 transition-all duration-300"
